...
import { rayFromPln } from '../../../core/inline/_ray';
import { isEmptyArr2, arrMakeFlat, arrMaxDepth } from '../../..//libs/util/arrs';
...
function _vector(__model__: GIModel, ents_arrs: TEntTypeIdx|TEntTypeIdx[]): Txyz|Txyz[] {
    if (getArrDepth(ents_arrs) === 1) {
        ...
        if (ent_type === EEntType.EDGE) {
            ...
            const start: Txyz = __model__.modeldata.attribs.query.getVertCoords(verts_i[0]);
            const end: Txyz = __model__.modeldata.attribs.query.getVertCoords(verts_i[1]);
