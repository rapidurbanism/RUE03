...
import { idsMake, getArrDepth, isEmptyArr, idsBreak, idsMakeFromIndicies } from '../../../libs/geo-info/id';
import { isEmptyArr2, arrMakeFlat } from '../../../libs/util/arrs';
// ================================================================================================
...
export function Edge(__model__: GIModel, entities: TId | TId[], edge_query_enum: _EEdgeMethod): TId|TId[] {
    ...
    if (__model__.debug) {
        if (entities !== null && entities !== undefined) {
            ents_arr = checkIDs(__model__, 'query.Edge', 'entities', entities, [ID.isIDL], [EEntType.EDGE]) as TEntTypeIdx[];
        }
    } else {
        if (entities !== null && entities !== undefined) {
            ents_arr = idsBreak(entities) as TEntTypeIdx[];
    ...
    return idsMakeFromIndicies(EEntType.EDGE, edges_i) as TId[];
}
