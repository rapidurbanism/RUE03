...
Certain data values generated by the _Neighbourhood_ script are inserted into the `Q` spreadsheet.
These values are mostly quantities and areas extracted from the spatial model. Each time a new
spatial model is generated, these values will be updated, which in turn will trigger the financial
model to be recalculated. 
